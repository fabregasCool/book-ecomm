...
export default function ProductCreateAndUpdatePage() {
  // useNavigate
  ...
  const [book, setBook] = useState(''); //l'image
      ...
      state // On demande s'il ya un "state", si oui, cela signifie que c'est une  page de mise à jour donc on utilise notre route de "update"
        ? await axios.put(
            `http://localhost:9000/api/products/update/${state._id}`,
            {
              ...
              price,
              countInStock,
              book: state.book, //On initialise avec "rien" comme ça la colonne "book" est créer dans la BD
        ...
        : //Dans le 2nd cas, il n'ya pas de "state" donc on crée un nouveau post
          await axios.post(
            `http://localhost:9000/api/products/create`,
            {
              ...
              price,
              countInStock,
              book: book, //On initialise avec "rien" comme ça la colonne "book" est créer dans la BD
              img: file ? imgUrl : '',
